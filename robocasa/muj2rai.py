...
def get_ply_path(name: str, obj_path: str, texture_path: str, root_path: str, out_path: str, scaling: float=1.0) -> str:

    obj_path = os.path.join(root_path, obj_path)
    texture_path = os.path.join(root_path, texture_path)
    ply_path = f"{name}.ply"

    ply_success = obj2ply(obj_path, os.path.join(out_path, ply_path), texture_path=texture_path, scale=scaling)
    if ply_success:
        return ply_path
    return None
                    ...
                    if body_name:
                        ...
                        for i, geom in enumerate(geoms):
                            ...
                            if geom_type == "mesh":
                                ...
                                ply_path = get_ply_path(f"{body_name}_geom_{i}", models[mesh_name], materials[material_name], root, out_path, scaling)
